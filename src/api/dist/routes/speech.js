...
speechRouter.post('/chat', async (req, res, next) => {
    try {
        const { channelId, roleId, history, members, relationships, workContext } = req.body;
        if (!roleId || !channelId) {
...
- Do NOT use quotes. Just output the raw sentence.`;
        ...
        const response = await provider.chat(systemPrompt, [{ role: 'user', content: historyText }]);
