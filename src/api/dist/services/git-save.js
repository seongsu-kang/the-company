...
export function getGitStatus(root) {
    ...
    for (const line of lines) {
        ...
        if (!SAVE_PATHS.some(p => file.startsWith(p)))
            continue;
        if (status.includes('?')) {
            untracked.push(file);
        }
...
export function gitSave(root, message) {
    if (!isGitRepo(root)) {
...
/** Get commit history */
export function gitHistory(root, limit = 20) {
    if (!isGitRepo(root))
        return [];
...
export function gitRestore(root, sha, paths) {
    if (!isGitRepo(root)) {
